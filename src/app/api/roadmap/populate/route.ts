...
export async function POST() {
  try {
    ...
    if (!startup) {
      return NextResponse.json({ error: "No startup found. Complete onboarding first." }, { status: 404 })
    }

    // Check if roadmap already has items
    const existingCount = await db.roadmapItem.count({
      where: { startupId: startup.id },
    })

    if (existingCount > 0) {
      return NextResponse.json({ error: "Roadmap already has tasks. Delete existing ones first or add tasks manually." }, { status: 400 })
    }
    ...
    return NextResponse.json({ message: "Default roadmap tasks created", count: defaultTasks.length }, { status: 201 })
  } catch (error) {
    console.error("Roadmap populate error:", error)
